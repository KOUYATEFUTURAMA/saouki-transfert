            ...
            <div class="card-body">
                <div class="bg-white rounded shadow-sm py-5 px-10 px-lg-20">
                    <table id="table" class="table table-bordered table-hover table-checkable dataTable no-footer dtr-inline"
                            ...
                            data-search="true"
                            data-toggle="table"
                            data-url="{{ url('parametre', ['action' => 'list-partenairs']) }}"
                            data-unique-id="id"
                ...
                <div class="overlay overlay-block">
                    <form id="formAjout" ng-controller="formAjoutCtrl" action="#">
                        <div class="modal-header">
                            <h5 class="modal-title">Gestion des partenaires</h5>
                        ...
                        <div class="modal-body">
                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="form-group">
                                        <label>Nom et pr&eacute;nom(s) du partenaire *</label>
                                        <input type="text" onkeyup="this.value = this.value.charAt(0).toUpperCase() + this.value.substr(1);" class="form-control" name="name" id="name" placeholder="Nom et prénom(s) du partenair" required>
                                    </div>
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="form-group">
                                        <label>Contact *</label>
                                        <input type="text" class="form-control" name="contact" id="contact" placeholder="Contact du partenaire" required>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-group">
                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="form-group">
                                        <label>Adress</label>
                                        <input type="text" class="form-control" name="adress" id="adress" onkeyup="this.value = this.value.charAt(0).toUpperCase() + this.value.substr(1);" placeholder="Adresse du partenair">
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            <div class="modal-content">
                ...
                <div class="overlay overlay-block">
                    <form id="formSupprimer" ng-controller="formSupprimerCtrl" action="#">
                        <div class="modal-body">
                            <input type="text" ng-hide="true" id="idPartenairDelete" value="@{{ partenair.id }}">
                            @csrf
                            <p class="text-center text-muted h5">Etes vous certain de vouloir supprimer cet enregistrement ?</p>
                            <p class="text-center h4">@{{ partenair.name}}</p>
                        </div>
                        <div class="modal-footer">
...
<script type="text/javascript">
    ...
    saoukiApp.controller('formAjoutCtrl', function ($scope) {
        $scope.populateForm = function (partenair) {
            $scope.partenair = partenair;
        };
    ...
    $(function () {
        ...
        $("#formAjout").submit(function (e) {
            ...
            var methode = 'POST';
            var url = "{{route('parametre.partenairs.store')}}";

            editerPartenairAction(methode, url, $(this), $(this).serialize(), $overlayBlock, $spinnerLg, $table);
        });
        ...
        $("#formSupprimer").submit(function (e) {
            ...
            var id = $("#idPartenairDelete").val();

            deleteAction('partenairs/' + id, $(this).serialize(), $overlayBlock, $spinnerLg, $table);
        });
    });

    function updateRow(idPartenair) {
        var $scope = angular.element($("#formAjout")).scope();
        var partenair =_.findWhere(rows, {id: idPartenair});
        $scope.$apply(function () {
            $scope.populateForm(partenair);
        });
        $("#id").val(partenair.id);
        $("#name").val(partenair.name);
        $("#contact").val(partenair.contact);
        $("#adress").val(partenair.adress);
        $("#country_id").val(partenair.country_id).trigger('change');
    
        $(".bs-modal-ajout").modal("show");
    }

    function deleteRow(idPartenair) {
    ...
    function editerPartenairAction(methode, url, $formObject, formData, $overlayBlock, $spinnerLg,$table) {
        jQuery.ajax({
                type: methode,
                url: url,
                cache: false,
